...
# -*- coding: utf-8 -*-
"""
Created on Thu May 28 12:28:03 2026
...
try:
    ...
    print(f"Rota görseli kaydedildi: {plot_path}")

except TimeoutError:
    print(f"UYARI: {plot_path} kaydedilemedi, kod devam ediyor.")
...
print("Tip 1 required edge:", len(required_tip1))
print("Tip 2 required edge:", len(required_tip2))
...
vehicle_access_col = {
    1: "avg_gecis",
    2: "ayg_gecis",   # şimdilik AYT aktif değil; ileride ayt_gecis eklenebilir
    3: "azu_gecis"
}
...
def evaluate_population(population, vehicle_id):
    ...
    print(f"\n{vehicle_name[vehicle_id]} başlangıç popülasyonu:")
...
costs_avg, fitness_avg, routes_avg_population = evaluate_population(
    population_avg,
    vehicle_id=1
)

costs_azu, fitness_azu, routes_azu_population = evaluate_population(
    population_azu,
    vehicle_id=3
)
...
def write_ga_kml(
    ...
    try:
        ...
        print(f"UYARI: KML yazılamadı, zaman aşımı: {output_path}")

    except Exception as e:
        print(f"UYARI: KML yazılamadı: {output_path} | {e}")
     ...
     print(f"UYARI: KML yazılamadı: {output_path} | {e}")
def write_ga_kml_by_route(route_df, vehicle_label):
...
# BELEDİYE SERVİS SIRASI KML ÇIKTILARI

write_ga_kml(
    municipality_df_1,
    vehicle_label="BELEDIYE_SEFER_1",
    output_path=os.path.join(
        kml_folder,
        "BELEDIYE_SEFER_1_shortest_path.kml"
    )
)

write_ga_kml(
    municipality_df_2,
    vehicle_label="BELEDIYE_SEFER_2",
    output_path=os.path.join(
        kml_folder,
        "BELEDIYE_SEFER_2_shortest_path.kml"))
...
# =========================
# 21. KDS OPERASYON ÖZETİ
# =========================

operation_window = "15:00-23:00"

ga_total_cost = final_best_cost_avg + final_best_cost_azu
ga_total_load = sum(e["demand"] for e in required_edges)
ga_total_route_count = len(final_best_routes_avg) + len(final_best_routes_azu)

# =========================
# 21.1 TEMEL OPERASYON BİLGİSİ
# =========================

operation_info_df = pd.DataFrame([
    {"Gösterge": "Pilot Bölge", "Değer": ", ".join(selected_neighborhoods)},
    {"Gösterge": "Operasyon Penceresi", "Değer": operation_window},
    {"Gösterge": "Aktif Araçlar", "Değer": "AVG + AZU"},
    {"Gösterge": "Depo Yapısı", "Değer": "Gerçek depo + sanal depo bağlantısı"}
])
...
workload_summary_df = pd.DataFrame([
    {"Gösterge": "Toplam Servis Edilecek Yol Sayısı", "Değer": len(required_edges)},
    {"Gösterge": "Tip 1 Servis Yolu Sayısı", "Değer": len(required_tip1)},
    {"Gösterge": "Tip 2 Servis Yolu Sayısı", "Değer": len(required_tip2)},
    {"Gösterge": "Tip 1 Toplam Talep", "Değer": sum(e["demand"] for e in required_tip1)},
    {"Gösterge": "Tip 2 Toplam Talep", "Değer": sum(e["demand"] for e in required_tip2)},
    {"Gösterge": "Toplam Talep", "Değer": ga_total_load}
])

# =========================
# 21.3 ARAÇ PERFORMANS ÖZETİ
# =========================

vehicle_performance_df = pd.DataFrame([
    {
        "Araç": "AVG",
        "Konteyner Tipi": "Tip 1",
        "Rota Sayısı": len(final_best_routes_avg),
        "Toplam Maliyet": final_best_cost_avg,
        "Toplam Yük": sum(e["demand"] for e in required_tip1)
    },
    {
        "Araç": "AZU",
        "Konteyner Tipi": "Tip 2",
        "Rota Sayısı": len(final_best_routes_azu),
        "Toplam Maliyet": final_best_cost_azu,
        "Toplam Yük": sum(e["demand"] for e in required_tip2)
    }
])
...
kds_general_summary_df = pd.DataFrame([
    {"Gösterge": "GA Toplam Maliyet", "Değer": ga_total_cost},
    {"Gösterge": "GA Toplam Yük", "Değer": ga_total_load},
    {"Gösterge": "GA Toplam Rota Sayısı", "Değer": ga_total_route_count}
])

print("\n===== 21.1 TEMEL OPERASYON BİLGİSİ =====")
print(operation_info_df)

print("\n===== 21.2 İŞ YÜKÜ ÖZETİ =====")
print(workload_summary_df)

print("\n===== 21.3 ARAÇ PERFORMANS ÖZETİ =====")
print(vehicle_performance_df)

print("\n===== 21.4 KDS GENEL ÖZET =====")
print(kds_general_summary_df)


# =========================
# 22. KDS ÇIKTILARINI KAYDET
# =========================
...
operation_info_df.to_csv(
    os.path.join(kds_output_folder, "operation_info.csv"),
    index=False
)

workload_summary_df.to_csv(
    os.path.join(kds_output_folder, "workload_summary.csv"),
    index=False
)

vehicle_performance_df.to_csv(
    os.path.join(kds_output_folder, "vehicle_performance.csv"),
    index=False
)

kds_general_summary_df.to_csv(
    os.path.join(kds_output_folder, "kds_general_summary.csv"),
    index=False
)

final_best_avg_df.to_csv(
    os.path.join(kds_output_folder, "avg_route_details.csv"),
    index=False
)

final_best_azu_df.to_csv(
    os.path.join(kds_output_folder, "azu_route_details.csv"),
    index=False
)

municipality_summary_df = pd.DataFrame([
    {"Gösterge": "Araç Kodu", "Değer": "AVG303"},
    {"Gösterge": "Araç Tipi", "Değer": "Tip 1"},
    {"Gösterge": "Hizmet Bölgesi", "Değer": "Atatürk + Adatepe + Çamlıkule"},
    {"Gösterge": "Sefer Sayısı", "Değer": 2},
    {"Gösterge": "Toplam Rota Maliyeti", "Değer": municipality_total_cost},
    {"Gösterge": "Toplam Toplanan Talep", "Değer": municipality_total_load},
    {"Gösterge": "Operasyon Penceresi", "Değer": "15:00-23:00"},
    {"Gösterge": "Depo Yapısı", "Değer": "Gerçek depo + sanal depo bağlantısı"}
])

municipality_summary_df.to_csv(
    os.path.join(kds_output_folder, "municipality_summary.csv"),
    index=False
)

municipality_df_1.to_csv(
    os.path.join(kds_output_folder, "municipality_route_1_details.csv"),
    index=False
)

municipality_df_2.to_csv(
    os.path.join(kds_output_folder, "municipality_route_2_details.csv"),
    index=False
)
